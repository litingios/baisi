...
@interface AVFoundationVC ()<AVCaptureFileOutputRecordingDelegate>{
...
- (void)viewDidLoad {
    ...
    self.navigationItem.rightBarButtonItem = [UIBarButtonItem itemWithImage:@"play-voice-stop" highImage:@"play-voice-stop" target:self action:@selector(tagClick)];
    [self setupUI];
    //录制的区域添加视频
...
- (void)setupUI{
    self.backView = [[UIView alloc]initWithFrame:CGRectMake(0, 0, iPhone_Width, iPhone_Height-500*HeightScale)];
    self.backView.backgroundColor = [UIColor clearColor];
    [self.view addSubview:self.backView];
    NSArray * arr = @[@"开始录制",@"结束录制",@"视频时长",@"播放视频"];
    [self createModel:arr];
}
...
- (void)createModel:(NSArray *)array{
    ...
    //方块的尺寸
    int maxColsCount = 2; //一行最多4列
    CGFloat buttonW = self.view.lt_width / maxColsCount;
    CGFloat buttonH = 100*HeightScale;
    ...
    for (NSInteger i = 0; i < count; i++) {
        ...
        button.lt_y = (i / maxColsCount) * (buttonH+60*HeightScale)+self.backView.lt_bottom+60*HeightScale;
        button.lt_width = buttonW - 2*WidthScale;
        button.lt_height = buttonH;
...
-(void)addGenstureRecognizer{
    ...
    [self.backView addGestureRecognizer:doubleTapGesture];
}
...
- (void)previewLayer{
    ...
    _captureVideoPreviewLayer.position = CGPointMake(self.backView.frame.size.width*0.5,self.backView.frame.size.height*0.5);
...
#define WeakSelf __weak typeof(self) weakSelf = self;
    ...
    dispatch_source_set_event_handler(_timer, ^{
        ...
        dispatch_async(dispatch_get_main_queue(), ^{
            ...
            [button setTitle:[NSString stringWithFormat:@"%d", startTimer++] forState:UIControlStateNormal];
        });
    });
