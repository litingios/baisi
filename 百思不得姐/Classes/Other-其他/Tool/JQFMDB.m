...
- (BOOL)jq_deleteTable:(NSString *)tableName dicOrModel:(id)parameters{
    BOOL flag;
    LTEssenceModel * model = (id)parameters;
    /*** 删除数据库中的数据 ****/
    flag = [self jq_deleteTable:tableName whereFormat:[NSString stringWithFormat:@"WHERE id = %@",model.id]];
    return flag;
}
